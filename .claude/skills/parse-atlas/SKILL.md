...
name: parse-atlas
description: >
  Knowledge base for parsing the Sky Atlas into the SAbR graph. Use when
...
| `s:transfer` (~32) | Mostly false positives (operational runbook steps: "Call mintUSDS Function", "Encode Transfer Function"). The genuine subset — Initial/Genesis Capital Allocations and the Liquidity Bootstrapping transfers — is now extracted by Pattern 18 shapes D/E. Prime TGE and "Initial Allocation Distribution" carry no figures (mechanics prose) — deliberately not extracted. |
...
| `multisig`            | —                      | Multisig detected via the structural five-child convention (Pattern 17). Entity id = root doc UUID. Meta: `{source, address, chain, threshold, threshold_doc_no, purpose_doc_no}`. Target of `signer_of` / `can_modify_signers_of`.                |
...
| `bridge`              | —                      | Cross-chain bridge component detected via the Validators + Quorum Requirement child pair (Pattern 21). Entity id = root doc UUID. Meta: `{source, component, network, quorum, quorum_doc_no}`. Target of `validator_of`.                          |
...
| `c304cb9f` (`A.2.2.10.1.1.1.6.2.1.3.1.0.6.1`) | Approved Deviations | **Genuine gap, extraction deferred** (triaged 2026-08-28, issue #339, atlas `1704409`). One row: `Liquidity Layer` / `Multisig Address` / `Approved Deviation` — an exception registry for the default Pattern 17 multisig signer/threshold config, keyed by multisig address rather than a named actor. The `Approved Deviation` cell is free-text prose ("Two (2) of four (4) signing requirement, with signers controlled by Phoenix Labs and VoteWizard"), not a structured field — same deferral shape as the Sky Direct Exposures rows above. A future extractor would resolve `Multisig Address` against the Pattern 17 multisig entity (by address) and attach the deviation prose as alt-config annotation on it. First populated this atlas bump. |
...
- **Signer shapes** (`parseSignerGroups`): (a) `"N (N) address(es) controlled by {Party}"` runs — the dominant shape; (b) bullet `"- {Party}: N signers"`; (c) `"{word} (N) controlled by {Party}"` runs without the word "address(es)" (a composition breakdown, e.g. "four (4) controlled by the Operational Executor Agent … and one (1) controlled by the Prime Agent") — a leading `"including at least (N) controlled by …"` sub-clause describes how one group's own count is internally composed, not additional signers, and is stripped before matching; `signerCompositionMismatch` cross-checks the parsed sum against a stated `"is N (N) signers"` total so a future rephrase that defeats the strip fails loudly instead of silently dropping a group; (d) `"… are controlled by {Party}. The specific signers will be specified in a future iteration…"` — the roster is deferred but the controlling party is real, so it's still emitted (with a null count) rather than dropped; (e) plain bullet roster after `"has the following signers"` (each = 1 signer, may be individuals like VoteWizard/LDR).
...
- **C. Accord grant authorizations:** docs titled `…Grant Authorization…` (directory docs starting "The documents herein" skipped). `"grant of {N} USDS per month to the {Recipient} from {Sender}'s Prime Treasury for a three (3) month period"`. Both `per month` and the `for a N month period` tail are optional — single-month/one-time grants drop the recurrence ("… grant of 800,000 USDS to the Grove Foundation from Grove's Prime Treasury for July 2026", "… one-time cash grant of 700,000 USDS to the Skybase Foundation from Skybase's SubProxy …"); source can be a Treasury or a SubProxy. `kind="authorization"`; `meta.period` from the title suffix ("December 2025"), `begin_date` from a separate "beginning on …" match, `meta.period_months` omitted when no month count is stated. Multi-grant docs only capture the first grant (known limitation).
- **D. Accord capital allocations:** docs whose title ENDS with `Initial Allocation` or `Genesis Capital Allocation` (suffix gate keeps "Initial Allocation Distribution" mechanics docs out; directory docs say "The **sub**documents herein" — `DIRECTORY_RE` covers both). Two content forms: `"The {kind} for {Party} is {N} USDS."` → one edge, sender `sky-core`, `kind="allocation"`, `allocation_type` initial/genesis_capital, `status="allocated"` (or `"planned"` + `meta.scheduled` when "included in the … Executive Vote"); and the enumerated form (A.2.8.2.5.2.2) `"Sky Core shall directly transfer (1) {N} USDS to X and (2) {M} USDS to Y"` → one edge per item, `meta.account` when the recipient string is an account of the resolved entity (e.g. "… SubProxy"). 10 edges today (all 8 accord agents + CCEA1 split across SubProxy and Core Council Buffer Multisig).
- **E. Executed budget transfers:** no title gate — any doc matching `"{Sender} has transferred {N million} {TOKEN} from the {Source Budget} to {Recipient}"`. The required amount+token core makes the sentence intrinsically monetary, and both endpoints are resolve-only (never created), so non-monetary "has transferred" prose can't emit. `kind="budget_transfer"`, `status="completed"`, `meta.source_budget`. 2 edges today (Liquidity Bootstrapping → Spark).
- **Party resolution extras:** prose drops the "Multisig" suffix ("the Core Council Buffer") — `resolveParty` retries `{name} Multisig` after the direct and SubProxy-stripping lookups.
