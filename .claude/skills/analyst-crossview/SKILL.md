---
name: analyst-crossview
description: >
  Methodology for building out the Atlas CrossView's analyst content — the
  /reports/crossview section (Shape, Concepts, Audit, Glossary) and its research
  docs. Triggered by phrases like "analyst crossview", "atlas crossview", "concept
  mining", "concept catalog", "chunk digest", "onboarding checklist", or any
  work on docs/crossview/*, src/lib/crossviewShape.ts, or
  src/components/crossview/*. Covers the locked design decisions, the
...
The Atlas CrossView is a scholarly layer over the 10,780-doc Sky Atlas: the *shape* of the
...
- `docs/plans/atlas-crossview.md` — product plan: the four faces, 7 staleness signals, phases.
- `docs/features/atlas-crossview/LOG.md` — append-only work log; bottom-up = latest state.
- `docs/atlas-map.md` — the chunk-taxonomy research the GROUPS seed came from.

**Deliverables map:**
- `docs/crossview/concepts.md` — the concept catalog (shipped in-app: Concepts tab, `?raw` import).
- `docs/crossview/concepts-audit.md` — its evidence-tier audit (shipped in-app: Audit tab).
...
- `src/lib/crossviewShape.ts` — the shape compute (chunk trees, weights, totals),
  derived client-side by `loadCrossView` (`src/lib/crossview.ts`).
...
2. **Inside agent artifacts `parentId` is unreliable** (heading depth caps at 6; the tree
   ...
   `scripts/lib/crossview-shape.mjs`). UUIDs are identity; doc_nos only in comments — except
   the spec-guaranteed structural suffixes (`.0.3.X`, `.0.4.X`, `.1.X`, `.varX`, `.0.6.X`,
   `NR-X`).
3. **The crossview shape is computed client-side, not shipped as an artifact.** There
   WAS a `public/crossview.json` (full artifact lifecycle + `SCHEMA_V` skew guard); it
   was folded away 2026-07-24 once measured at 2 MB — a pure projection of docs.json
   that cost more to ship than to compute (Stale Dates pattern). `loadCrossView`
   (`src/lib/crossview.ts`) now derives `CrossViewData` from `loadAtlas` + `loadGlossary`
   per data-source base — which also makes the crossview work over previews. The
   compute (`src/lib/crossviewShape.ts`, `computeCrossView`) is pure and server-importable;
...
5. The curated taxonomy (UUID roots) is `GROUPS` in `src/lib/crossviewShape.ts`; recursion
   ...
   parent, not the scenario (3 docs; locked in `crossviewShape.test.ts` until deliberately fixed).
...
Promoting concept groups into `crossview.json`/UI is a decision made WITH the user, never
unilaterally.

## Session practices

- Commit in reasonable chunks with why-messages; stage and hand the commit command to the
  user (never commit yourself). Append to `docs/features/atlas-crossview/LOG.md` at every
