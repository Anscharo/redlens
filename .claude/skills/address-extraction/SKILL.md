...
The capture now admits a dot but never a **sentence-ending** one. `NAME_CHAR` defines that precisely: letter, `.`, optional closing quote/bracket, space, capitalised word. A dot between letters (`Sky.money`, `U.S.A`) or touching a digit (`v1.5`, `0.75`) is part of the token and stays in the name; banning `.` outright cost those names their first half. The capitalised word is deliberately not required to be Capital-then-lowercase — an acronym starts a sentence too, and crossing that boundary costs the whole label (the captured clause fails `isPlausibleName`, so the answer becomes `null`) where stopping at it costs at most a leading acronym.
