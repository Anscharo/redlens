  ...
  return (
    <div className="flex-1 px-6 py-8">
      ...
      {/* Concepts gets its own (wider, lg+ only) row so a left TOC column fits
          beside the max-w-3xl article measure. Shape widens so the chunk-map
          treemap can fill a larger square (labels were clipping in the 3xl
          column). Glossary/audit keep the plain centered 3xl column. Below lg
          (TOC hidden), concepts collapses back to the identical max-w-3xl
          mx-auto layout. At xl+ a third column (CrossViewTopicIndex, the
          right-hand "Topics" panel) joins in — both side columns hide
          themselves below their own breakpoint via their own classes, so this
          row just widens the cap to fit all three at xl. */}
      <div
        className={
          tab === "concepts"
