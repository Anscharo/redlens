...
describe("TraceRowView", () => {
  it("renders a tool row with formatted byte size and success arrow", () => {
    const e: TraceRow = { name: "atlas_query", args: { search: "gov" }, ok: true, bytes: 2048, round: 1 };
    render(<TraceRowView e={e} />);
  ...
  it("shows the error arrow and a pending byte count when ok is false and bytes is null", () => {
    const e: TraceRow = { name: "atlas_get", args: { id: "abc" }, ok: false, bytes: null, round: 1 };
    render(<TraceRowView e={e} />);
  ...
  it("summarizes non-string args as JSON and formats sub-1KB byte counts", () => {
    const e: TraceRow = { name: "atlas_query", args: { limit: 5 }, ok: true, bytes: 100, round: 1 };
    render(<TraceRowView e={e} />);
  ...
  it("shows a fired fact by its summary, with no call arrow or byte size", () => {
    const e: TraceRow = { name: "glossary", args: {}, ok: true, bytes: null, kind: "fact", summary: "2 glossary definitions", round: 0 };
    const { container } = render(<TraceRowView e={e} />);
