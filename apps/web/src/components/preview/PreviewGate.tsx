...
export function PreviewGate({ id, routerBase }: { id: string; routerBase: string }) {
  ...
  if (phase === "failed") {
    ...
    if (code === "app-not-installed") {
      ...
      // A bare-SHA id names no repo (see previewRepo); fall back to generic copy.
          ...
          {installUrl ? (
            <>
              ...
              <p className="text-sm" style={{ color: "var(--tan-3)" }}>
                If you own or administer {repoName}, install the app: on GitHub's page choose{" "}
                <strong>Only select repositories</strong> and pick {repoName} only — it needs nothing else — then
                reload. Otherwise, ask the repo owner/admin to do that.
              </p>
              <a
                href={installUrl}
                target="_blank"
            ...
            <p className="text-sm" style={{ color: "var(--tan-3)" }}>
              Ask the repo owner/admin to install it on {repoName} only, then reopen this link.
            </p>
          )}
          <a href={`${import.meta.env.BASE_URL}preview`} className="text-sm" style={{ color: "var(--accent)" }}>
            ← back to previews
