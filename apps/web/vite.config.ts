...
export default defineConfig(() => {
  ...
  return {
    base,
    // Atlas artifacts are built into the REPO ROOT public/ by the pipeline, and
    // the server's config.publicDir plus the Dockerfile's public -> dist symlink
    // both resolve them there. Only this app's own sources moved into apps/web,
    // so Vite still reads and writes at the root.
  ...
  plugins: [
    {
      ...
      transformIndexHtml(html, ctx) {
        ...
        try {
          const parsed = JSON.parse(readFileSync(fileURLToPath(new URL("../../public/docs.json", import.meta.url)), "utf8"));
