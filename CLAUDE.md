...
- **Staged delivery (docs/plans/chat-staged-delivery.md, decided 2026-08-06)**: replace stream-then-check with a status-stage UX ("Looking for evidence" → … → "Preparing final report") where only the final verified answer renders. Not started; land the loop-exhaustion compose guard first.
