...
### Chat skills (auto-injected context)

A **skill** is a block of context the chat injects into a turn only when the turn calls for it — deterministic, pure code, no LLM call, no network, so a miss costs microseconds and injects nothing. The registry is `src/server/skills/registry.ts`; the contract is `skills/types.ts`. Today: glossary definitions, entity rows, concept censuses (all three matching the question text), and app documentation (`skills/features.ts`, which reads the Features guide and also fires from the `/features` page). All of them ride ONE synthetic tool round in the transcript, so the verifier, quote-grounding, and citation checks consume whatever they inject as ordinary turn evidence. **Add a skill by writing one and adding it to `SKILLS` — never by editing `chat.ts`.** `CHAT_PREFETCH=0` disables the whole registry.

Two distinctions the chat must never blur, and where each is enforced: **this chat vs the whole web app** (the features skill's `chat` / `app` split) and **the Atlas vs our extraction** — the atlas is the source documents; entities, relations, roles, addresses, params, censuses and every report built on them are RedLens's parse of them. That one is stated globally in the system prompt's "Entity traversal (live graph)" section, and reinforced by the features skill's `vocabulary` block and `CENSUSES_NOTE`.
...
- **Mechanical gate**: `scripts/lib/report-citations.mjs` detects normative claims lacking an in-context citation. Run `pnpm cite:check <report.md>` on any file/site report draft before shipping it. The Notion report publisher wires the same detector and **refuses to publish** uncited claims (that integration ships with the Notion publishing pipeline; `--allow-uncited` is the only, deliberately loud, escape hatch). Do not weaken or bypass this gate to get a report out — an uncited claim means find the citation or soften the claim to a description of what the atlas *says*.

**Component work follows [components.build](https://www.components.build)** — the open component specification — adapted to this codebase in three skills: `react-components` (creating/updating a component: one-element-per-component, extending `React.ComponentProps`, compound `Root`/`Trigger`/`Content` composition, semantic HTML + ARIA baselines, `data-state`), `react-state` (where state lives: local → `useUrlState` → context → `useSyncExternalStore` → `useLoaded`/workers, plus the house rule that components stay fully controlled with domain-named callbacks), and `react-review` (auditing a frontend diff against those principles). The spec's class-merging chapters (`cn`, `tailwind-merge`, CVA) and its npm/registry distribution chapters are deliberately **not** adopted — RedLens is an app, not a published component library, and styling is covered by `ui-look-and-feel`.
