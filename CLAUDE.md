...
### Monthly Settlement Cycle accounting

Two rules govern how MSC workbooks are read. Both were violated once and cost real accuracy; neither is obvious from the sheet.

- **A Prime's supply-side revenue is `prime_agent_revenue − cof`, never `Σ` per-venue `Profit to Grove`.** `settlements.ts` exposes it as `supplyKept()`; the upstream definition is `settlement-cycle`'s `src/settle/load/summary.py`, and it is what the published `summary.md` prints. The venue-row sum silently drops prime-level revenue that has no venue row (PSM3 sUSDS appreciation) plus the sUSDS spread reimbursement — on Spark that ran ~$1.0M/month, up to 11.6% of the cycle. The workbook's own `Comparison (Grove-style "Profit to Grove")` block prints `prime_agent_revenue` and `−CoF` as its addends but totals them with `Σ Profit to Grove`, so **it does not foot**; `reconcile()` reports the gap as `dComparisonFoot` rather than trusting the total. Per-venue `profitToGrove` is still correct *as a venue breakdown* (the Sankey uses it) — it is only the sum that must not become a headline.
- **Cost of funds IS the money sent to Sky, not a fourth flow.** `settlement-cycle` derives it as `cof = sky_revenue − sde_revenue`, so CoF and "To Sky" differ only by Sky Direct Exposure — identical for Obex, <0.5% apart for Spark. Render it as a component of "To Sky" (`HeadlineFigure.component`), never as a peer of it, or readers double-count Sky's take.

`parseSummary` **throws** on a Summary label it cannot match (`REQUIRED_SUMMARY_ROWS`), mirroring what `parseVenues` already did for its headers. This is not pedantry: `cellNum` returns 0 for a missing cell, and 0 is a legitimate CoF (Keel and Skybase settle at zero), so a silent 0 is indistinguishable from a real one. Upstream already spells it two ways — `CoF on utilized` in the xlsx, `prime cost of funds` in `summary.md`. `reconcile().dCof` is the check that bites: `Σ CoF alloc − Σ Spread Reimb ≡ headline.cof` (the allocation is of the *gross* CoF, with the spread refunded inside `Profit to Sky`). `dSky` and `dP2G` restate the venue table against totals derived from that same table, so they are ~0 by construction and detect almost nothing.
