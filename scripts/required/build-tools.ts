...
import { EXTERNAL_TOOLS } from "../../src/server/chat/tools/external-tools.ts";
...
const tools = [
  ...ATLAS_TOOLS.map((t) => ({ name: t.name, desc: t.description })),
  ...EXTERNAL_TOOLS.map((t) => ({ name: t.name, desc: t.description })),
