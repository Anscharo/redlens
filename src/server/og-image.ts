...
function cardNode(title: string, docNo: string) {
  ...
  return {
    ...
    props: {
      ...
      children: [
        ...
        // Doc number, small gray. Omitted (empty div collapses) when absent.
        ...(docNo
          ? [{ type: "div", props: { style: { display: "flex", fontSize: 30, fontWeight: 500, color: TAN_3, letterSpacing: "0.06em", marginBottom: "14px" }, children: docNo } }]
          : []),
...
export async function renderOgImage(title: string, docNo = ""): Promise<Buffer | null> {
  try {
    const [{ default: satori }, { Resvg }] = await Promise.all([import("satori"), import("@resvg/resvg-js")]);
    const svg = await satori(cardNode(title, docNo) as never, { width: WIDTH, height: HEIGHT, fonts: loadFonts() });
