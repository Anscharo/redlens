...
export function buildSystemPrompt(ix: Indexes, ctx?: PageContext): string {
  ...
  return [
    ...
    "- `atlas_report` — curated one-call rollups too big to assemble by hand. kind='multisigs' returns every multisig with threshold, signer orgs + counts, modification authorities, purpose, and provenance (use for multisig/security-review questions); kind='primitive_matrix' returns the agent × primitive-subtype matrix marking universal vs optional primitives with missing-agent lists (use for primitive-structure questions).",
