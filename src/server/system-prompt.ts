...
export function buildSystemPrompt(ix: Indexes, ctx?: PageContext): string {
  ...
  return [
    ...
    "- `atlas_report` — curated one-call rollups too big to assemble by hand. kind='multisigs' returns every multisig with threshold, signer orgs + counts, modification authorities, purpose, and provenance (use for multisig/security-review questions); kind='primitive_matrix' returns the agent × primitive-subtype activation matrix (engaged = Active|Completed vs Inactive), classing each primitive universal/optional/dormant — missing_agents means Inactive (present but not engaged), not absent; use for primitive-structure questions; kind='facilitator_responsibilities' returns every Operational/Core Facilitator responsibility grouped by category (universal duties, Core/Operational duties, per-Executor assignments, Active Data maintenance, process steps) with duty text and attribution — use for 'what is a Facilitator responsible for' questions; kind='govops_responsibilities' is the GovOps counterpart (role definitions, Operational/Core GovOps duties, per-Executor assignments, Active Data maintenance, process steps) — use for 'what is GovOps responsible for' questions; kind='rewards' returns the per-agent integrator reward rollup (operational chain plus Distribution Reward and Integration Boost primitives with each Instance/Invocation's status, reward code/partner, address, chain, cadence) — use for reward-program and integrator questions; kind='active_data' returns one row per Active Data doc (controller, resolved Responsible Party with evidence, prime→executor→facilitator/govops chain, approving Facilitator, update process) — use for 'who is responsible for / maintains this Active Data' questions.",
