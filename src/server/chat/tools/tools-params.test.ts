...
test("matches every query token of 3+ chars across name + owner + doc_no combined", () => {
  ...
  const res = atlasParams(fixtureIx(rows), { query: "keel maxamount", limit: 25 }) as { rows: Array<{ uuid: string }> };
...
test("a row must satisfy EVERY query token, not just one", () => {
  ...
  const res = atlasParams(fixtureIx(rows), { query: "keel spark", limit: 25 }) as { rows: unknown[] };
...
test("row shape omits internal num/source fields", () => {
  const res = atlasParams(fixtureIx([row({ uuid: "a" })]), { query: "maxamount", limit: 25 }) as { rows: Array<Record<string, unknown>> };
...
test("caps results at limit and flags truncated only when rows were actually cut", () => {
  ...
  const capped = atlasParams(ix, { query: "maxamount", limit: 2 }) as { count: number; truncated?: boolean; rows: unknown[] };
  expect(capped.rows.length).toBe(2);
  expect(capped.count).toBe(2);
  expect(capped.truncated).toBe(true);

  const full = atlasParams(ix, { query: "maxamount", limit: 25 }) as { count: number; truncated?: boolean; rows: unknown[] };
...
test("an empty, whitespace-only, or all-short-token query errors instead of matching everything", () => {
  ...
  expect((atlasParams(ix, { query: "", limit: 25 }) as { error?: string }).error).toBeDefined();
  expect((atlasParams(ix, { query: "   ", limit: 25 }) as { error?: string }).error).toBeDefined();
  expect((atlasParams(ix, { query: "at of", limit: 25 }) as { error?: string }).error).toBeDefined(); // every token <3 chars
});
...
test("buildIndexes: a KV row extracted from real doc content is findable by name, with owner resolved", () => {
  ...
  const res = atlasParams(ix, { query: "maxamount", limit: 10 }) as { rows: Array<Record<string, unknown>> };
