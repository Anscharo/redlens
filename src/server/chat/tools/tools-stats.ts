...
// computed from the same semantic doc_no tree as /reports/crossview
// (src/lib/crossviewShape.ts). Fills a gap the primitive tools can't: subtree
...
import { computeCrossView, GROUPS, type ChunkNode, type GroupSpec } from "../../../lib/crossviewShape.ts";

// Presentation trim: the full trees carry a ref for nearly every doc (the
// reason the 2 MB crossview.json artifact was folded away) — a tool answer
...
export function statsSection(ix: Indexes): Record<string, unknown> {
  ...
  const lib = computeCrossView({ atlasCommit: ix.meta.atlasCommit ?? "unknown", nodes, glossaryTerms }, groups);
