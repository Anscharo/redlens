...
// (distinct from crossviewShape's internal completeness diff) —
...
const A2_2 = "fcde2604-a138-4c1b-9d9a-14895835c907"; // Primitive spec crossview root == the "except" entry
...
function makeGroupsIx(opts: { includeA2: boolean }): Indexes {
  const docs: AtlasNode[] = [
    ...
    node(A2_2, "A.2.2", "Article"), // "Primitive spec crossview" root, also the complement's "except" entry
...
test("complementOf group resolves children(A.2) minus the except set when the anchor exists", () => {
  ...
  const primitiveLib = s.groups.find((g) => g.title === "Primitive spec crossview");
  ...
  expect(primitiveLib).toMatchObject({ title: "Primitive spec crossview", doc_no: "A.2.2", docs: 1 });
});
...
test("complementOf group is dropped entirely when its anchor (A.2) is missing, unlike roots groups", () => {
  ...
  expect(s.groups.find((g) => g.title === "Primitive spec crossview")).toBeDefined();
});
