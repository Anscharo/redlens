...
// alternative to reading the in-doc II.7 list (CrossViewIndexList.tsx). Parses
// the same II.7 section straight out of the raw markdown source (mirrors
// CrossViewToc.tsx / crossviewHeadings.ts's parse-the-source approach — no DOM
...
export function CrossViewTopicIndex() {
  const headings = useMemo(() => extractHeadings(conceptsRaw), []);
  const entries = useMemo(() => parseCrossViewIndex(conceptsRaw, headings), [headings]);
