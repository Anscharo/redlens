...
function wrap() {
  const { hook } = memoryLocation({ path: "/reports/library", record: true });
  return ({ children }: { children: React.ReactNode }) => <Router hook={hook}>{children}</Router>;
}

afterEach(cleanup);

describe("LibraryChunkTree", () => {
  it("renders a leaf row with no expand affordance and a disabled toggle", () => {
    ...
    render(<LibraryChunkTree tree={tree} atlasTotal={100} />, { wrapper: wrap() });
  ...
  it("includes a 'smaller sections' remainder segment only when remainder >= 5", () => {
    ...
    const { container, unmount } = render(<LibraryChunkTree tree={bigRemainder} atlasTotal={100} />, { wrapper: wrap() });
    fireEvent.click(screen.getByRole("button", { name: "Parent" }));
    ...
    const { container: container2 } = render(<LibraryChunkTree tree={smallRemainder} atlasTotal={100} />, { wrapper: wrap() });
    fireEvent.click(screen.getByRole("button", { name: "Parent" }));
  ...
  it("shows the doc_no at depth 0 only when rootDocNo is set, and always at deeper depths", () => {
    ...
    const { unmount } = render(<LibraryChunkTree tree={tree} atlasTotal={100} />, { wrapper: wrap() });
    ...
    render(<LibraryChunkTree tree={tree} atlasTotal={100} rootDocNo />, { wrapper: wrap() });
