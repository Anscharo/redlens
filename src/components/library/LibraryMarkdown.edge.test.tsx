...
// Split from LibraryMarkdown.test.tsx to keep files near the ~150-line
...
describe("LibraryMarkdown — hash scroll effect", () => {
...
describe("LibraryMarkdown — nested markup inside a unit-opener bold lead", () => {
...
describe("LibraryMarkdown — h3/h4 heading rendering and sticky mode", () => {
  ...
  it("adds the library-sticky-headings class when sticky is true", async () => {
    const raw = "## Title\n\nBody.";
    const { container } = render(<LibraryMarkdown raw={raw} sticky />);
    await screen.findByText("Body.");
    expect(container.querySelector(".library-sticky-headings")).toBeInTheDocument();
  });
});

describe("LibraryMarkdown — non-doc-ref link and code passthrough", () => {
