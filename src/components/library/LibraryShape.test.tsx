...
function wrap() {
  const { hook } = memoryLocation({ path: "/reports/library/shape", record: true });
...
describe("LibraryShape", () => {
  it("renders the header stats line", () => {
    render(<LibraryShape data={DATA} />, { wrapper: wrap() });
  ...
  it("downloads the chunk tree as CSV via the DownloadCsvButton, matching libraryChunksToCSV", () => {
    render(<LibraryShape data={DATA} />, { wrapper: wrap() });
