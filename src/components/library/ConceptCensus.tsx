...
// line (see LibraryMarkdown.tsx's splitByCensusMarkers) — every censused
...
export function ConceptCensus({ slug }: { slug: string }) {
  ...
  const exportCsv = () => {
    track("report_export", { report: `library-concepts-census-${slug}`, format: "csv", scope: "full", row_count: result.members.length });
