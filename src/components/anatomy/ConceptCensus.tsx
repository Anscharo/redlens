...
// Interleaved into docs/anatomy/concepts.md via a `:::census <slug>` marker
// line (see AnatomyMarkdown.tsx's splitByCensusMarkers) — every censused
...
// anatomy plan. Computed client-side from loadAtlas() (no new artifact),
// mirroring loadAnatomy()'s pattern — see src/lib/conceptsCensus.ts.
//
...
export function ConceptCensus({ slug }: { slug: string }) {
  ...
  const exportCsv = () => {
    track("report_export", { report: `anatomy-concepts-census-${slug}`, format: "csv", scope: "full", row_count: result.members.length });
