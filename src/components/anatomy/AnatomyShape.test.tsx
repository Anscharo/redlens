...
function wrap() {
  const { hook } = memoryLocation({ path: "/reports/anatomy/shape", record: true });
...
describe("AnatomyShape", () => {
  it("renders the header stats line", () => {
    render(<AnatomyShape data={DATA} />, { wrapper: wrap() });
  ...
  it("downloads the chunk tree as CSV via the DownloadCsvButton, matching anatomyChunksToCSV", () => {
    render(<AnatomyShape data={DATA} />, { wrapper: wrap() });
