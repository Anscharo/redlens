...
import { extractHeadings, partitionHeadings } from "../../lib/anatomyHeadings";
import { parseAnatomyIndex } from "../../lib/anatomyIndex";
import { buildDocRefResolver, type DocRefResolver } from "../../lib/docRefResolver";
import { buildComponents } from "./anatomyMarkdownComponents";
import { AnatomyIndexList } from "./AnatomyIndexList";

// A `:::census <slug>` line, alone on its own line, interleaves a live
// <ConceptCensus> block into the curated prose (docs/anatomy/concepts.md).
// A `:::index` … `:::endindex` block wraps the II.7 Topics list — stripped
// out of the plain-markdown pass entirely and swapped for <AnatomyIndexList>
// so its "→ Group N" targets render as hash links instead of plain text
// (see anatomyIndex.ts). Only concepts.md uses either marker;
...
// Shared renderer for the anatomy's curated markdown docs (Concepts, Audit) —
...
// behavior (see buildComponents in anatomyMarkdownComponents.tsx) so the page
// never blocks on it.
export function AnatomyMarkdown({ raw, sticky = false }: { raw: string; sticky?: boolean }) {
  ...
  // see anatomyHeadings.ts. Memoized on `raw` only (stable across resolver
  ...
  // parseAnatomyIndex locates the II.7 section itself via its own heading
  // regex), shared with the right-hand AnatomyTopicIndex.tsx panel so both
  // surfaces agree on every target's slug.
  const indexEntries = useMemo(() => parseAnatomyIndex(raw, headings), [raw, headings]);
  ...
  return (
    <div className={`atlas-md anatomy-md text-sm text-tan-2${sticky ? " anatomy-sticky-headings" : ""}`}>
      {segments.map((seg, i) =>
        seg.kind === "census" ? (
          <ConceptCensus key={i} slug={seg.slug} />
        ) : seg.kind === "index" ? (
          <AnatomyIndexList key={i} entries={indexEntries} />
        ) : (
          <ReactMarkdown
            key={i}
