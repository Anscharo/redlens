...
function wrap() {
  const { hook } = memoryLocation({ path: "/reports/anatomy/glossary", record: true });
...
describe("AnatomyGlossary", () => {
  ...
  it("scrolls the term matching the current URL hash into view once terms resolve", async () => {
    ...
    try {
      ...
      window.history.pushState(null, "", "/reports/anatomy/glossary");
    }
  });
  ...
  it("passes the data-source base to loadGlossary and refetches when the base changes", async () => {
    ...
    const { rerender } = render(<AnatomyGlossary />, { wrapper: wrap() });
    await screen.findByText("Accord");
    rerender(<AnatomyGlossary />);
