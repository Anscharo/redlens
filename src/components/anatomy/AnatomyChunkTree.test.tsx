...
import type { ChunkNode } from "../../lib/anatomy";
...
import { AnatomyChunkTree } from "./AnatomyChunkTree";

function wrap() {
  const { hook } = memoryLocation({ path: "/reports/anatomy", record: true });
  return ({ children }: { children: React.ReactNode }) => <Router hook={hook}>{children}</Router>;
}

afterEach(cleanup);

describe("AnatomyChunkTree", () => {
  it("renders a leaf row with no expand affordance and a disabled toggle", () => {
    ...
    render(<AnatomyChunkTree tree={tree} atlasTotal={100} />, { wrapper: wrap() });
  ...
  it("includes a 'smaller sections' remainder segment only when remainder >= 5", () => {
    ...
    const { container, unmount } = render(<AnatomyChunkTree tree={bigRemainder} atlasTotal={100} />, { wrapper: wrap() });
    fireEvent.click(screen.getByRole("button", { name: "Parent" }));
    ...
    const { container: container2 } = render(<AnatomyChunkTree tree={smallRemainder} atlasTotal={100} />, { wrapper: wrap() });
    fireEvent.click(screen.getByRole("button", { name: "Parent" }));
  ...
  it("shows the doc_no at depth 0 only when rootDocNo is set, and always at deeper depths", () => {
    ...
    const { unmount } = render(<AnatomyChunkTree tree={tree} atlasTotal={100} />, { wrapper: wrap() });
    ...
    render(<AnatomyChunkTree tree={tree} atlasTotal={100} rootDocNo />, { wrapper: wrap() });
