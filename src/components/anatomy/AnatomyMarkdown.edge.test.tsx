...
// Split from AnatomyMarkdown.test.tsx to keep files near the ~150-line
...
describe("AnatomyMarkdown — hash scroll effect", () => {
  it("scrolls the hash target into view once headings have rendered", async () => {
    ...
    render(<AnatomyMarkdown raw={raw} />);
...
describe("AnatomyMarkdown — nested markup inside a unit-opener bold lead", () => {
...
describe("AnatomyMarkdown — h3/h4 heading rendering and sticky mode", () => {
  ...
  it("adds the anatomy-sticky-headings class when sticky is true", async () => {
    const raw = "## Title\n\nBody.";
    const { container } = render(<AnatomyMarkdown raw={raw} sticky />);
    await screen.findByText("Body.");
    expect(container.querySelector(".anatomy-sticky-headings")).toBeInTheDocument();
  });
});

describe("AnatomyMarkdown — non-doc-ref link and code passthrough", () => {
