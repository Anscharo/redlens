...
// alternative to reading the in-doc II.7 list (AnatomyIndexList.tsx). Parses
// the same II.7 section straight out of the raw markdown source (mirrors
// AnatomyToc.tsx / anatomyHeadings.ts's parse-the-source approach — no DOM
...
const PANEL_TOP = 80; // matches AnatomyToc's TOC_TOP — same sticky offset math

export function AnatomyTopicIndex() {
  const headings = useMemo(() => extractHeadings(conceptsRaw), []);
  const entries = useMemo(() => parseAnatomyIndex(conceptsRaw, headings), [headings]);
