...
import { loadAnatomy, type AnatomyData } from "../../lib/anatomy";
import { useDataSource } from "../../lib/dataSource";
import { AnatomyShape } from "./AnatomyShape";
import { AnatomyGlossary } from "./AnatomyGlossary";
import { AnatomyConcepts, AnatomyAudit } from "./AnatomyConcepts";
import { AnatomyToc } from "./AnatomyToc";
import { AnatomyTopicIndex } from "./AnatomyTopicIndex";

export type AnatomyTab = "shape" | "concepts" | "audit" | "glossary";

const TABS: { tab: AnatomyTab; label: string; to: string }[] = [
  { tab: "shape", label: "Shape", to: ROUTES.REPORTS_ANATOMY },
  { tab: "concepts", label: "Concepts", to: ROUTES.REPORTS_ANATOMY_CONCEPTS },
  { tab: "audit", label: "Audit", to: ROUTES.REPORTS_ANATOMY_AUDIT },
  { tab: "glossary", label: "Glossary", to: ROUTES.REPORTS_ANATOMY_GLOSSARY },
];

export function AnatomyPage({ tab }: { tab: AnatomyTab }) {
  useDocumentTitle("Atlas Anatomy: Sky Atlas by Redline");
  const { base } = useDataSource();
  const [data, setData] = useState<AnatomyData | null>(null);
  const [error, setError] = useState<string | null>(null);
  useEffect(() => {
    track("report_view", { report: "anatomy" });
  }, []);
  useEffect(() => {
    let on = true;
    loadAnatomy(base)
  ...
  return (
    <div className="flex-1 px-6 py-8">
      <div className="max-w-3xl mx-auto">
        <p className="mono text-xs text-tan-3 mb-1">anatomy</p>
      ...
      {/* Concepts gets its own (wider, lg+ only) row so a left TOC column fits
          ...
          xl+ a third column (AnatomyTopicIndex, the right-hand "Topics"
        ...
        <div className={tab === "concepts" ? "max-w-3xl lg:min-w-0 lg:flex-1" : undefined}>
          {tab === "glossary" ? (
            ...
            <p className="text-sm mono" style={{ color: "var(--error-text)" }}>
              anatomy data failed to load: {error}
            </p>
          ) : !data ? (
            <p className="text-sm mono text-tan-3">loading…</p>
          ) : (
            <AnatomyShape data={data} />
          )}
        </div>
        {tab === "concepts" && <AnatomyTopicIndex />}
      </div>
    </div>
  );
}
