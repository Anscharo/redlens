...
import { flattenChunkTree, anatomyChunksToCSV } from "../../lib/anatomy";
import { AnatomyChunkTree } from "./AnatomyChunkTree";
import { AnatomyTreemap } from "./AnatomyTreemap";
import { DownloadCsvButton } from "../reports/DownloadCsvButton";
import { Link } from "../Link";
import { atlasHref } from "../../lib/routes";

export function AnatomyShape({ data }: { data: AnatomyData }) {
  const chunkRowCount = flattenChunkTree(data.chunkTree, data.totals.docs).length;
  const buildCsv = () => anatomyChunksToCSV(data.chunkTree, data.totals.docs);
  return (
    <div>
      <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
        ...
        <DownloadCsvButton
          report="anatomy"
          filename="atlas-anatomy-chunks.csv"
      ...
      <section className="mb-8">
        ...
        <AnatomyTreemap tree={data.chunkTree} atlasTotal={data.totals.docs} />
      </section>
      <section className="mb-8">
        <h2 className="text-base font-semibold mb-1" style={{ color: "var(--tan)" }}>
          Doc mass by scope
        ...
        <AnatomyChunkTree tree={data.scopeTree} atlasTotal={data.totals.docs} rootDocNo />
      </section>
      <section className="mb-8">
        <h2 className="text-base font-semibold mb-1" style={{ color: "var(--tan)" }}>
          Chunk tree
        ...
        <AnatomyChunkTree tree={data.chunkTree} atlasTotal={data.totals.docs} />
      </section>
      <section className="mb-8">
        <h2 className="text-base font-semibold mb-1" style={{ color: "var(--tan)" }}>
          Overlay chunks
