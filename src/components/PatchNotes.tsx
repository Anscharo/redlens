...
export function PatchNotes() {
  ...
  return (
    <section className="mt-12">
      <h2 className="text-sm font-semibold text-tan mb-4">Recent improvements</h2>
      <div className="flex flex-col gap-4">
        {groups.map((g) => (
          <article key={g.date}>
            <time dateTime={g.date} className="text-xs text-tan-3">
              {formatPatchDate(g.date)}
            </time>
            <ul className="mt-1 list-disc list-inside text-xs text-tan-2 leading-relaxed">
              {g.items.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
