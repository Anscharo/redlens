...
import { REPORT_TITLES, ROUTES, reportHref } from "../lib/routes";

// Pages tracked centrally because their title is a constant, not something the
// page derives. /radar/<slug> is deliberately absent — RadarPage records that
// one itself, where the actor's name is available.
...
export function visitTitleFor(location: string): string | null {
...
// these routes sync their filter state to the URL, and the /history page shows
// what was set). The `report_view` analytics event is fired by ReportShell (once
// per report page, once its data lands); this is the separate browser-local log,
...
export function usePageVisitTracking(location: string): void {
  ...
  useEffect(() => {
    ...
    if (!label) return;
    const path = location.startsWith(REPORT_PREFIX)
      ? reportHref(location.slice(REPORT_PREFIX.length))
      : location;
    void recordVisit({ path, label, base, params: search });
