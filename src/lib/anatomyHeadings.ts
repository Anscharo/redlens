// Parses the ## / ### heading outline directly out of a curated anatomy
// markdown source (docs/anatomy/concepts.md, concepts-audit.md) — ONE pass
// over the raw text, shared by both the heading-id rehype plugin
// (rehypeHeadingIds.ts) and the Concepts TOC (AnatomyToc.tsx). Deriving both
...
export interface AnatomyHeading {
  level: 2 | 3;
  text: string;
  slug: string;
}
...
export function extractHeadings(raw: string): AnatomyHeading[] {
  const slugger = makeSlugger();
  const out: AnatomyHeading[] = [];
 ...
 *  heading list across AnatomyMarkdown's `:::census`-split segments. */
 ...
 * order. AnatomyMarkdown renders each `:::census`-split segment with its own
 ...
 */
export function partitionHeadings(headings: AnatomyHeading[], segmentTexts: string[]): AnatomyHeading[][] {
  let cursor = 0;
