...
// consuming slugs from a pre-computed ordered list (see libraryHeadings.ts)
...
// LibraryMarkdown.tsx) must pass EXACTLY the headings that occur in the
...
export function rehypeHeadingIds(headings: LibraryHeading[]) {
  return () => (tree: Root) => {
    let cursor = 0;
