...
// from the synthetic-fixture unit tests in anatomyShape.test.ts).

import { describe, it, expect, vi } from "vitest";
import fs from "node:fs";
import path from "node:path";
import type { AtlasNode } from "../types";
import { computeAnatomy, GROUPS } from "./anatomyShape";
...
describe("computeAnatomy against the real atlas", () => {
  ...
  it("leaves no A.1/A.2 article unclaimed on the current atlas shape", () => {
    ...
    const lib = computeAnatomy({ atlasCommit: docsFile.atlasCommit ?? "unknown", nodes, glossaryTerms: 0 });
