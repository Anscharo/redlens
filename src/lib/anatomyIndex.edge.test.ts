// Split from anatomyIndex.test.ts to keep files near the ~150-line
...
import { parseAnatomyIndex } from "./anatomyIndex";
import type { AnatomyHeading } from "./anatomyHeadings";

const marker = (body: string) => `:::index\n${body}\n:::endindex\n`;

describe("parseAnatomyIndex — unresolved family targets", () => {
  it("resolves a range target's slug to null (but keeps kind 'category') when its family has no GROUP_SECTION_HEADING entry", () => {
    ...
    const entries = parseAnatomyIndex(raw, []);
  ...
  it("leaves a bare-family target unresolved when the recognized family's section heading is absent from `headings`", () => {
    ...
    const headings: AnatomyHeading[] = [{ level: 2, text: "Some Other Section", slug: "some-other-section" }];
    const entries = parseAnatomyIndex(raw, headings);
