// Parses the ## / ### heading outline directly out of a curated crossview
// markdown source (docs/crossview/concepts.md, concepts-audit.md) — ONE pass
// over the raw text, shared by both the heading-id rehype plugin
// (rehypeHeadingIds.ts) and the Concepts TOC (CrossViewToc.tsx). Deriving both
...
export interface CrossViewHeading {
  level: 2 | 3;
  text: string;
  slug: string;
}
...
export function extractHeadings(raw: string): CrossViewHeading[] {
  const slugger = makeSlugger();
  const out: CrossViewHeading[] = [];
 ...
 *  heading list across CrossViewMarkdown's `:::census`-split segments. */
 ...
 * order. CrossViewMarkdown renders each `:::census`-split segment with its own
 ...
 */
export function partitionHeadings(headings: CrossViewHeading[], segmentTexts: string[]): CrossViewHeading[][] {
  let cursor = 0;
