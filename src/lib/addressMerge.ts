// Pure merge of the two address artifacts into one AddressInfo per address.
// Split out of apps/web/src/lib/addresses.ts (which owns the fetch + the
// atlas-versioned base cache) so the server-side onchain-addresses report tool
// can reuse the exact same merge rules instead of re-deriving them.
import type { AddressInfo } from "../types";
import { EXPLORER } from "./explorer";

export type AtlasAddr = {
  chain: string;
...
export function mergeAddressInfo(
  atlas: Record<string, AtlasAddr>,
  onChain: Record<string, OnChainAddr>,
