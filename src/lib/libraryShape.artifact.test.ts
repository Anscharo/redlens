...
// from the synthetic-fixture unit tests in libraryShape.test.ts).

import { describe, it, expect, vi } from "vitest";
import fs from "node:fs";
import path from "node:path";
import type { AtlasNode } from "../types";
import { computeLibrary, GROUPS } from "./libraryShape";
...
describe("computeLibrary against the real atlas", () => {
  ...
  it("leaves no A.1/A.2 article unclaimed on the current atlas shape", () => {
    ...
    const lib = computeLibrary({ atlasCommit: docsFile.atlasCommit ?? "unknown", nodes, glossaryTerms: 0 });
