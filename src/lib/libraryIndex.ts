...
// concepts.md source (mirrors libraryHeadings.ts's parse-the-source
// approach — no DOM query) into {topic, targets} entries shared by the
// in-doc linkified list (libraryMarkdownComponents.tsx) and the right-hand
// topic index panel (LibraryTopicIndex.tsx). One parse, two renders, so
...
// the unit-opener id assignment in libraryMarkdownComponents.tsx so the two
...
export type LibraryIndexTargetKind = "unit" | "category" | "unresolved";

export interface LibraryIndexTarget {
  label: string;
  slug: string | null;
  kind: LibraryIndexTargetKind;
}

export interface LibraryIndexEntry {
  topic: string;
  targets: LibraryIndexTarget[];
}

/** Slug for a specific concept unit, e.g. groupRefSlug("Instruments", "1") ===
 *  "instruments-1" — the exact id libraryMarkdownComponents.tsx stamps onto
...
// The `:::index` / `:::endindex` pair is the same marker LibraryMarkdown.tsx
...
function sectionSlugFor(family: string, headings: LibraryHeading[]): string | null {
  const headingText = GROUP_SECTION_HEADING[family];
  if (!headingText) return null;
  return headings.find((h) => h.text === headingText)?.slug ?? null;
}

function parseTarget(raw: string, headings: LibraryHeading[]): LibraryIndexTarget {
...
export function parseLibraryIndex(raw: string, headings: LibraryHeading[]): LibraryIndexEntry[] {
...
export type GroupedTargets =
  ...
  | { mode: "full"; targets: LibraryIndexTarget[] };
 ...
 *  full label. Used by both LibraryIndexList (in-doc) and LibraryTopicIndex
 *  (right panel) so the two agree on when to compact. */
export function groupTargetsForDisplay(targets: LibraryIndexTarget[]): GroupedTargets {
  if (targets.length > 1) {
