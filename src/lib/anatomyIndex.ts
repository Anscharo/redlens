...
// concepts.md source (mirrors anatomyHeadings.ts's parse-the-source
// approach — no DOM query) into {topic, targets} entries shared by the
// in-doc linkified list (anatomyMarkdownComponents.tsx) and the right-hand
// topic index panel (AnatomyTopicIndex.tsx). One parse, two renders, so
...
// the unit-opener id assignment in anatomyMarkdownComponents.tsx so the two
...
export type AnatomyIndexTargetKind = "unit" | "category" | "unresolved";

export interface AnatomyIndexTarget {
  label: string;
  slug: string | null;
  kind: AnatomyIndexTargetKind;
}

export interface AnatomyIndexEntry {
  topic: string;
  targets: AnatomyIndexTarget[];
}

/** Slug for a specific concept unit, e.g. groupRefSlug("Instruments", "1") ===
 *  "instruments-1" — the exact id anatomyMarkdownComponents.tsx stamps onto
...
// The `:::index` / `:::endindex` pair is the same marker AnatomyMarkdown.tsx
...
function sectionSlugFor(family: string, headings: AnatomyHeading[]): string | null {
  const headingText = GROUP_SECTION_HEADING[family];
  if (!headingText) return null;
  return headings.find((h) => h.text === headingText)?.slug ?? null;
}

function parseTarget(raw: string, headings: AnatomyHeading[]): AnatomyIndexTarget {
...
export function parseAnatomyIndex(raw: string, headings: AnatomyHeading[]): AnatomyIndexEntry[] {
  const body = extractIndexSectionRaw(raw);
  const entries: AnatomyIndexEntry[] = [];
...
export type GroupedTargets =
  ...
  | { mode: "full"; targets: AnatomyIndexTarget[] };
 ...
 *  full label. Used by both AnatomyIndexList (in-doc) and AnatomyTopicIndex
 *  (right panel) so the two agree on when to compact. */
export function groupTargetsForDisplay(targets: AnatomyIndexTarget[]): GroupedTargets {
  if (targets.length > 1) {
