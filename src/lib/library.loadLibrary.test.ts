...
import type { LibraryInputs } from "./libraryShape";

// loadLibrary composes loadAtlas + loadGlossary + computeLibrary behind a
// base-keyed cache (mirrors ConceptCensus.test.tsx's approach to loadAtlas,
// and LibraryGlossary.test.tsx's approach to loadGlossary). computeLibrary's
// own logic is exercised thoroughly in libraryShape.test.ts (its default
// GROUPS reference real production UUIDs a synthetic fixture won't have) —
// this file only checks that loadLibrary wires its two data sources and the
// per-base cache correctly, so computeLibrary is stubbed to echo its inputs.
...
const computeLibraryCalls = vi.fn();
vi.mock("./libraryShape", () => ({
  computeLibrary: (inputs: LibraryInputs) => {
    computeLibraryCalls(inputs);
    return {
...
beforeEach(() => {
  ...
  computeLibraryCalls.mockClear();
  loadAtlasImpl = () => Promise.resolve({ docs: docsFixture, atlasCommit: "sha1234" });
  loadGlossaryImpl = () => Promise.resolve(glossaryFixture);
});

describe("loadLibrary", () => {
  it("composes loadAtlas + loadGlossary into a computeLibrary projection", async () => {
    const data = await loadLibrary(freshBase());
  ...
  it("fetches independently per distinct base", async () => {
    ...
    await loadLibrary(baseA);
    await loadLibrary(baseB);
  ...
  it("does not cache a rejection — a later call retries instead of hanging on the failed promise", async () => {
    ...
    await expect(loadLibrary(base)).rejects.toThrow("network down");

    loadAtlasImpl = () => Promise.resolve({ docs: docsFixture, atlasCommit: "recovered" });
    const data = await loadLibrary(base);
