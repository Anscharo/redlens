...
// Pure compute for the Atlas CrossView shape (CrossViewData) — an O(n) projection
// of the docs bundle, derived client-side in loadCrossView (src/lib/crossview.ts).
// This replaced the public/crossview.json build artifact: the artifact was 2 MB
...
// computeCrossView. "Support processes" uses the complement form (children(A.2)
...
export const GROUPS: GroupSpec[] = [
  ...
  { name: "Primitive spec crossview", roots: ["fcde2604-a138-4c1b-9d9a-14895835c907" /* A.2.2 */] },
...
export interface CrossViewData {
  atlasCommit: string;
...
export interface CrossViewInputs {
  atlasCommit: string;
...
export function computeCrossView(
  { atlasCommit, nodes, glossaryTerms }: CrossViewInputs,
  ...
  if (unclaimed.length > 0) {
    console.warn(
      `[crossviewShape] ${unclaimed.length} article(s) under A.1/A.2 not claimed by any GROUPS entry — ` +
        `${unclaimed.map((n) => n.doc_no).join(", ")}. Shown under "Ungrouped"; extend GROUPS in src/lib/crossviewShape.ts.`,
    );
  }
