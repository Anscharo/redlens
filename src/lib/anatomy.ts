...
import { computeAnatomy } from "./anatomyShape";
import { toCSV } from "./csv";

// ChunkNode/AnatomyData live in anatomyShape.ts (DOM-free, server-importable);
// re-exported here so frontend imports keep one entry point.
export type { ChunkNode, AnatomyData } from "./anatomyShape";
import type { ChunkNode, AnatomyData } from "./anatomyShape";
...
export function anatomyChunksToCSV(tree: ChunkNode[], atlasTotal: number): string {
...
// Derived in place from the docs bundle + glossary — there is no anatomy.json
// artifact (it was a pure 2 MB projection of docs.json; see anatomyShape.ts).
...
// version skew to guard against. Keyed by data-source base, so the anatomy
// works over previews too.
const cache = new Map<string, Promise<AnatomyData>>();

export function loadAnatomy(base: string = liveAtlasBase()): Promise<AnatomyData> {
