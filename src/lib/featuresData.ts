  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        what: "The Radar front page opens with a cross-prime Monthly Settlement Cycle view: a three-stage flow chart — the line-item sources on the left (cost of funds, Sky Direct Exposure, supply-side kept, each demand-side series), one bar per Prime in the middle, and Sky on the right split by what each Prime sent it — with a PIES pill that swaps in the pie chart (Sky as a central pie, each Prime orbiting as a pie of what it received, a supply-side loss as a hole in the middle, an arrow to its Sky wedge) — next to a timeline of what each Prime sent to Sky, month by month, stacked in the Primes' colors.",
        how: [
          "Open Radar; the chart (titled Sky System Settlements) shows the latest settled month across all Primes as the sankey, with the color key and reading guide under it; the SANKEY / PIES pills beside the title switch styles. Primes keep one fixed order and color everywhere on the overview — the Atlas's own Prime Agent order (Spark, Grove, Keel, Skybase, Obex, Pattern, Osero, Launch Agent 7).",
          "The `sankey` / `pies` pills beside the chart's title switch between a three-stage sankey of the month and the pies. The sankey reads: the line-item sources on the left, one bar per Prime in the middle fed by ribbons in the same colors, and Sky on the right split by Prime and by type. The source column is grouped by where the money comes from: the Prime's own line items first (cost of funds, Sky Direct Exposure, supply-side kept), then the demand-side series under the heading OWED BY SKY, gathered by a bracket in Sky's own blue — so Sky appears at both ends, which is what the cycle does: the demand side is owed by Sky (A.2.4.1.2.2.1.1.1) and the supply side is owed to it (A.2.4.1.2.2.1.1.2). A supply-side loss shows as a striped gap on a Prime's bar where no ribbon arrives — more leaves that bar for Sky than came in. The choice is in the URL (`?view=pies`), so a link can open either.",
          "Scroll — or pinch on a trackpad — over either chart to zoom in on it, which is how you get at the sankey's hairline ribbons and the smallest pies; drag to pan once zoomed, and double-click or press the Reset zoom button to go back. From the keyboard, tab to the chart and use + and − to zoom, the arrow keys to pan, and 0 or Escape to go back. On the sankey the zoom stays put as you step through months; the pies re-crop each month, so they start again at full view.",
          "Click a month column in the To Sky by month chart to switch the settlements chart and the figures under it, press the ▶ play button over the month on the To Sky card to step through the months (it opens paused on the latest), or use the ← and → keys — on a Prime's settlement page too. In the left nav, a Prime with settlement workbooks opens into Info and Settlements; on a settlements page every Prime is open, so you can step straight to another Prime's chart.",
          "On the pies every circle is what that party RECEIVED and nothing else: a Prime's is its supply-side kept plus the demand-side series, Sky's is cost of funds plus Sky Direct Exposure, split by Prime. Two arrows run between each Prime and Sky in opposite lanes — what it owed Sky, and the demand-side Sky owed it (A.2.4.1.2.2.1.1.1).",
          "Hover a slice, the loss hole, or a Sky wedge for its figure, a Prime's name for what it received, or either arrow for its total; the To-Sky arrow also lists its cost of funds and Sky Direct Exposure components. Hover a timeline segment to light that Prime up on the pies.",
          "Click a Prime's pie to open its settlement page for the month you had selected.",
        ],
        note: "Every circle — Sky's pie, each Prime's, each loss hole — is on one area scale, so a pie's ring area is exactly what that party received and a pie twice the area is twice the money; only a Prime too small to draw is held at a minimum size. To Sky is a pass-through (cost of funds + Sky Direct Exposure), not the Prime's revenue, so it is never in a Prime's pie — it lives in the arrow and in Sky's. A Prime can receive less than nothing: a supply-side loss bigger than what it kept shows as a hole filling almost the whole pie. Hidden when settlements.json has not been built. Figures are Soter Labs OEA calculations, not the on-chain GovOps spell.",
      },
      {
        ...
        how: [
          ...
          "The settlement charts are headed by the window's three running totals — what the Prime sent to Sky, what it kept supply-side, and what it earned demand-side — and show a year of cycles at a time; a Prime with more gets ‹ › arrows beside that heading to page back through older ones.",
