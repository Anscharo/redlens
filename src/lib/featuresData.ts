    ...
    features: [
      ...
      },
      {
        ...
        how: [
          ...
          "The `orbit` / `flow` pills beside the chart's title switch it to a three-stage flow chart of the same month: the line-item sources on the left, one bar per Prime in the middle fed by ribbons in the same colors, and Sky on the right split by Prime and by type. The source column is grouped by where the money comes from: the Prime's own line items first (cost of funds, Sky Direct Exposure, supply-side kept), then the demand-side series under the heading OWED BY SKY, and that second group hangs off a Sky node in the left gutter — so Sky appears at both ends, which is what the cycle does: the demand side is owed by Sky (A.2.4.1.2.2.1.1.1) and the supply side is owed to it (A.2.4.1.2.2.1.1.2). A supply-side loss shows as a striped gap on a Prime's bar where no ribbon arrives — more leaves that bar for Sky than came in. The choice is in the URL (`?view=flow`), so a link can open either.",
          "Scroll — or pinch on a trackpad — over the flow chart to zoom in on it, which is how you get at the hairline ribbons; drag to pan once zoomed, and double-click or press the Reset zoom button to go back. The zoom stays put as you step through months.",
          ...
          "On the orbital chart every pie is what that party RECEIVED and nothing else: a Prime's is its supply-side kept plus the demand-side series, Sky's is cost of funds plus Sky Direct Exposure, split by Prime. Two arrows run between each Prime and Sky in opposite lanes — what it owed Sky, and the demand-side Sky owed it (A.2.4.1.2.2.1.1.1).",
          "Hover a slice, the loss hole, or a Sky wedge for its figure, a Prime's name for what it received, or either arrow for its total; the To-Sky arrow also lists its cost of funds and Sky Direct Exposure components. Hover a timeline segment to light that Prime up on the orbital chart.",
