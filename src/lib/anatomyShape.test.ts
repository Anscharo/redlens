...
import { computeAnatomy, GROUPS, SCOPE_A1_UUID, SCOPE_A2_UUID, type GroupSpec } from "./anatomyShape";
...
const lib = (() => {
  ...
  try {
    return computeAnatomy({ atlasCommit: "abc1234", nodes, glossaryTerms: 3 }, FIXTURE_GROUPS);
  } finally {
    warn.mockRestore();
  }
})();
...
describe("computeAnatomy", () => {
  it("treats a falsy (empty) content string as zero bytes rather than throwing", () => {
    ...
    const out = computeAnatomy({ atlasCommit: "x", nodes: n, glossaryTerms: 0 }, []);
...
describe("GROUPS seed hardening", () => {
  it("complement rule: a new, never-enumerated A.2 article self-heals into the group", () => {
    ...
    const groups: GroupSpec[] = [
      { name: "Primitive spec anatomy", roots: ["id-A.2.2"] },
      { name: "Support processes", complementOf: "id-A.2", except: ["id-A.2.2"] },
    ];
    const out = computeAnatomy({ atlasCommit: "x", nodes: n, glossaryTerms: 0 }, groups);
  ...
  it("resolves A.1/A.2 by UUID, not doc_no literal — survives a renumbering", () => {
    ...
    const groups: GroupSpec[] = [{ name: "Primitive spec anatomy", roots: ["id-A.2.2"] }]; // B.1.1 never enumerated
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const out = computeAnatomy({ atlasCommit: "x", nodes: n, glossaryTerms: 0 }, groups);
    const ungrouped = out.chunkTree.find((g) => g.title === "Ungrouped");
