...
function fixture() {
  ...
  const atlas = makeAtlasBundle([sparkRoot, groveRoot, sparkToken, sparkTokenParam, groveToken, groveAccord, sparkTokenInvo]);
  ...
  return { atlas, graph, sparkToken, sparkTokenParam, groveToken };
}
...
describe("findCousinDocs", () => {
  ...
  it("resolves a doc inside an instance subtree via the doc_no ancestor chain", () => {
    const { atlas, graph, sparkTokenParam, groveToken } = fixture();
