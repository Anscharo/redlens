// Split from crossviewIndex.test.ts to keep files near the ~150-line
...
import { parseCrossViewIndex } from "./crossviewIndex";
import type { CrossViewHeading } from "./crossviewHeadings";

const marker = (body: string) => `:::index\n${body}\n:::endindex\n`;

describe("parseCrossViewIndex — unresolved family targets", () => {
  it("resolves a range target's slug to null (but keeps kind 'category') when its family has no GROUP_SECTION_HEADING entry", () => {
    ...
    const entries = parseCrossViewIndex(raw, []);
  ...
  it("leaves a bare-family target unresolved when the recognized family's section heading is absent from `headings`", () => {
    ...
    const headings: CrossViewHeading[] = [{ level: 2, text: "Some Other Section", slug: "some-other-section" }];
