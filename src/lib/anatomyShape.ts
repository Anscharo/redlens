...
// Pure compute for the Atlas Anatomy shape (AnatomyData) — an O(n) projection
// of the docs bundle, derived client-side in loadAnatomy (src/lib/anatomy.ts).
// This replaced the public/anatomy.json build artifact: the artifact was 2 MB
...
// computeAnatomy. "Support processes" uses the complement form (children(A.2)
...
export const GROUPS: GroupSpec[] = [
  ...
  { name: "Primitive spec anatomy", roots: ["fcde2604-a138-4c1b-9d9a-14895835c907" /* A.2.2 */] },
...
// Types live here (not in anatomy.ts) so the server can import this module
...
export function computeAnatomy(
  { atlasCommit, nodes, glossaryTerms }: AnatomyInputs,
  ...
  if (unclaimed.length > 0) {
    console.warn(
      `[anatomyShape] ${unclaimed.length} article(s) under A.1/A.2 not claimed by any GROUPS entry — ` +
        `${unclaimed.map((n) => n.doc_no).join(", ")}. Shown under "Ungrouped"; extend GROUPS in src/lib/anatomyShape.ts.`,
    );
  }
