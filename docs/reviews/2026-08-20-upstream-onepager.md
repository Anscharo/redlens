...
**Re-verified 2026-08-26** against `settlement-reports@main` as fetched 2026-08-24 — a later snapshot than the pin above, same 36 workbooks, same months. Finding 1 reproduces to the cent: every figure in its table, including the $7,290,483.28 January–July divergence, re-derived through a separate parser path with no shared code with the original pass (it surfaced there as an independent basis check, before this note was consulted). Finding 3 still stands — our parser maps the sheet's "Period inflow" to a position delta precisely because it is `value_eom − value_som`. **None of the three has been fixed upstream as of that snapshot.**

**Not in question:** the settlement math is internally consistent on every report — that is, the published files agree with `settlement-cycle` at the pinned commits. The Prime-side and Sky-side Summary blocks add up; `summary.md`'s supply-side revenue equals `prime_agent_revenue` minus Cost of Funds (CoF); the sum of venue revenue plus PSM3 sUSDS appreciation equals `prime_agent_revenue` within 2¢; and the settlement send (`sky_total_accrual.py`) computes the prime's supply share as `prime_agent_revenue − (sky_revenue − sde_revenue)` — the same `par − CoF`. The three items below are presentation defects in the published files: numbers a reader will take at face value and reconstruct incorrectly.

**Not assessed:** conformance to the Sky Atlas itself. Everything above compares the workbooks to the pipeline that generated them, which is a narrower question than whether the pipeline implements what the Atlas dictates. One point is worth a confirmation: the Atlas defines the interest base — utilized USDS — with five deductions (A.2.4.2.1.2), one of which is USDS attributable to a Sky Direct Exposure, whereas the pipeline reaches its figure by computing `CoF = sky_revenue − sde_revenue` (`sky_total_accrual.py`) with no SDE term in the utilized balance itself. The two are algebraically the same, `cof = (utilized − SDE) × rate`, **provided** `sde_revenue` is the borrow rate applied to the SDE balance rather than realised SDE yield. If it is the latter they diverge, and the difference would be material only for Grove, where SDE is large. This is a question, not a finding.
