...
navigable **library** — an educational/wiki/analysis layer inside RedLens that lets someone:
...
A new app section, `/library`, with four faces:

| Page | Purpose |
|---|---|
| **Shape** (`/library`) | Visual weight map — treemap/sunburst of chunks, grouped by taxonomy, sized by doc mass (and later by activity). The "you are here" overview. |
| **Contents** (`/library/contents`) | The distilled table of contents: chunk-level, not raw-tree-level. Each entry links to a chunk page and into the reader. |
| **Chunk pages** (`/library/chunk/<slug>`) | One page per chunk: digest (curated prose), key rules, member docs, entities involved, edges to other chunks, weight stats, staleness badges. |
| **Glossary** (`/library/glossary`) | All defined terms (we already build `glossary.json` — 100+ terms with aliases), each with definition, source pointer, and usage locations. |
...
```jsonc
{
  "id": "primitive-spec-library",        // stable slug, curated
  "group": "spec",                        // taxonomy group (see atlas-map.md §2)
  "title": "Sky Primitives (spec library)",
...
### Curated digest content — `docs/library/chunks/<slug>.md` (P2+)
...
- `docs/library/toc.md` — generated chunk-aware table of contents with weights + pointers.
...
hide content** — the library's job is to *flag* for human judgment, not to prune.

## Phases

- **P0 — Research & seed artifacts** (now): plan doc, `atlas-shape.mjs` prototype,
  `docs/library/{toc,shape}.md` generated, glossary census. No UI.
- **P1 — Chunk registry**: curated slug/taxonomy seed + `build-library.mjs` emitting
  `public/library.json`; snapshot test; census guard for new-structure drift (mirror the
  existing census pattern).
- **P2 — Library UI, read-only**: `/library` Shape (treemap), Contents, Glossary pages
  consuming `library.json` + existing `glossary.json`. Follow app conventions (URL-synced
...
- **Treemap library**: nothing heavy — either hand-rolled SVG (chunk count is small, ~40
...
- **2026-07-20** — Plan written. P0 executed: `scripts/aux/atlas-shape.mjs` prototype,
  generated `docs/library/toc.md` + `docs/library/shape.md`, glossary census. Taxonomy
