...
**The lexical ancestor is deleted (2026-09-22).** `findLowOverlapCitations` scored word overlap on the same (sentence, cited doc) pairs on every turn, was persisted inside the `round_checks` row, and was read by nothing. With the Jev check live and ahead of it on every wrong-doc class (same title 71% vs 50%, cited-elsewhere 92% vs 59%, at a comparable false-flag rate), it was removed from `runDeterministicChecks`/`CheckReport` and from the eval. Its numbers in the tables above are the only record of it. The segmentation it had been the only test of (`claimSegments`' trailing-citation fold) now has its own test in `verify-checks.test.ts`. The notes below, written before measurement, still describe it as dark and shipping.
