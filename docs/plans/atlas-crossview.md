...
A new app section, `/crossview`, with four faces:

| Page | Purpose |
|---|---|
| **Shape** (`/crossview`) | Visual weight map — treemap/sunburst of chunks, grouped by taxonomy, sized by doc mass (and later by activity). The "you are here" overview. |
| **Contents** (`/crossview/contents`) | The distilled table of contents: chunk-level, not raw-tree-level. Each entry links to a chunk page and into the reader. |
| **Chunk pages** (`/crossview/chunk/<slug>`) | One page per chunk: digest (curated prose), key rules, member docs, entities involved, edges to other chunks, weight stats, staleness badges. |
| **Glossary** (`/crossview/glossary`) | All defined terms (we already build `glossary.json` — 100+ terms with aliases), each with definition, source pointer, and usage locations. |
...
### `public/crossview.json` — the chunk registry (P1)
...
> base (`src/lib/crossviewShape.ts`, called from `loadCrossView`) — the Stale Dates
...
Built by a new pass `scripts/required/build-crossview.mjs` (prototype starts life as
`scripts/aux/atlas-shape.mjs`). For each chunk:

```jsonc
{
  "id": "primitive-spec-crossview",        // stable slug, curated
  "group": "spec",                        // taxonomy group (see atlas-map.md §2)
  "title": "Sky Primitives (spec crossview)",
...
### Curated digest content — `docs/crossview/chunks/<slug>.md` (P2+)
...
- `docs/crossview/toc.md` — generated chunk-aware table of contents with weights + pointers.
- `docs/crossview/shape.md` — generated weight tables/diagrams (the "shape of the Atlas").
...
markdown once `/crossview` shipped against `crossview.json`/`glossary.json` directly, and it
...
hide content** — the crossview's job is to *flag* for human judgment, not to prune.

## Phases

- **P0 — Research & seed artifacts** (now): plan doc, `atlas-shape.mjs` prototype,
  `docs/crossview/{toc,shape}.md` generated, glossary census. No UI.
- **P1 — Chunk registry**: curated slug/taxonomy seed + `build-crossview.mjs` emitting
  `public/crossview.json`; snapshot test; census guard for new-structure drift (mirror the
  existing census pattern).
- **P2 — CrossView UI, read-only**: `/crossview` Shape (treemap), Contents, Glossary pages
  consuming `crossview.json` + existing `glossary.json`. Follow app conventions (URL-synced
...
- **Treemap crossview**: nothing heavy — either hand-rolled SVG (chunk count is small, ~40
...
- **2026-07-20** — Plan written. P0 executed: `scripts/aux/atlas-shape.mjs` prototype,
  generated `docs/crossview/toc.md` + `docs/crossview/shape.md`, glossary census. Taxonomy
