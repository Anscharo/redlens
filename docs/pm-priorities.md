...
| 2 | **Search & reader quality** | "Search-first" is the core value prop — if search misses or mis-ranks, the product fails at its one job. | MiniSearch full-content + phrase/address/field filters live. Tokenization is shared (`src/lib/searchOptions.ts`); both surfaces `loadJSON` the same `search-index.json`. Query parsing, snippets, and ranking still diverge between the reader worker and MCP/chat. No MiniSearch relevance eval (`eval-golden-questions.ts` is chat, not ranking). | Land the remaining `src/lib/search/` core (`shared-search-core.md`) so reader + MCP rank identically. |
