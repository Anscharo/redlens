...
See `docs/plans/atlas-anatomy.md` for the roadmap. The P0 seed markdown (`docs/anatomy/{toc,shape,glossary}.md`,
generated by `scripts/aux/atlas-shape.mjs`) was removed once `/anatomy` shipped against
`public/anatomy.json` + `public/glossary.json` directly — nothing read the generated markdown
and it wasn't wired into any build script, so it went stale. The Shape/Contents/Glossary
`/anatomy` tabs are the live equivalent.
...
| A.2 The Support Scope | 1,355 | Primitive spec anatomy + support processes (rulebook/spec) |
...
### 2.4 The primitive spec anatomy (class definitions) — A.2.2
...
1. **Spec/instance split**: A.2.2 is the class anatomy; each prime artifact carries a full
...
- **2026-07-20** — Initial map: scope skeleton, doc-mass census, primitive spec anatomy,
  prime artifact anatomy (verified on Spark), chunk taxonomy v1, entity-layer summary.
- **2026-07-20 (b)** — Anatomy plan written (`docs/plans/atlas-anatomy.md`); P0 executed:
