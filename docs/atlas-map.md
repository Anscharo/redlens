...
See `docs/plans/atlas-library.md` for the roadmap. The P0 seed markdown (`docs/library/{toc,shape,glossary}.md`,
generated by `scripts/aux/atlas-shape.mjs`) was removed once `/library` shipped against
`public/library.json` + `public/glossary.json` directly — nothing read the generated markdown
and it wasn't wired into any build script, so it went stale. The Shape/Contents/Glossary
`/library` tabs are the live equivalent.
