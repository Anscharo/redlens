...
NOT follow the tree. Mission + method: the `analyst-crossview` skill
(`.claude/skills/analyst-crossview/SKILL.md`). Signatures re-run live against the
...
`docs/crossview/concepts-audit.md`'s rewrite plan), rendered here as a small
...
- **Detection signature** — entity_type `primitive` (15 subtypes × 8 agents = 120 entities); spec crossview at `fcde2604` /A.2.2.
...
because the codes were opaque. `docs/crossview/concepts-audit.md` and
`docs/features/atlas-crossview/LOG.md` are historical records and still use the
