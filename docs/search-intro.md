...
Let's look at both how the search system is prepared and indexed and secondly at the cycle from keystroke to result.
...
When a visitor arrives on atlas.redline.support the gzipped atlas files are fetched -- search-index.json by the search worker, docs-shallow/deep by the atlas worker, address files on the main thread, relations.json by the graph worker and again on the main thread (loadGraph) for search's entity rows -- and then used through-out the app.
...
9. while this is happening, our graph (relations.json, via loadGraph on the main thread) is also filtered for matches to the term
10. results from search worker and relations.json are rendered.


## Why?

* Docs is split so that the reader can render the first few levels without waiting for the entire doc file to download
