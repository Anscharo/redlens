# Atlas CrossView — work log

Append-only. Newest entry LAST. Every commit on `atlas-crossview` gets a note here; a
...
Branched `atlas-crossview` off `main` (post risk-report-rendering merge, atlas db87434).
...
- `docs/atlas-map.md`: chunk taxonomy v1. Key findings: 69% of docs live in A.6 agent
  ...
  artifact crossview Introduction/Sky Primitives/Omni Documents → families → primitives →
...
- `docs/plans/atlas-crossview.md`: product plan — /crossview with Shape / Contents / Chunk
  pages / Glossary; 7 staleness signals; phases P0 (seed) → P4 (scholarship).

### Commits

- **7e5bed76** "Add Atlas CrossView section: shape, contents, and glossary pages"
  P0 + first UI. Generated docs/crossview/{toc,shape,glossary}.md via scripts/aux/atlas-shape.mjs;
  /crossview routes + CrossViewPage tabs + Shape/Contents/Glossary pages; homepage card;
  patch-notes bullet; `public/crossview.json` added to .gitignore (atlas-artifact convention).
- **7633830f** "Wire crossview.json into the build pipeline and live-update path"
  User asked how updates flow; answer: worker → sync_state.atlas_sha → in-process updater
  → per-sha bundle → SSE. crossview.json was deploy-only + flat-root-loaded (two staleness
  bugs). Fixed: scripts/required/build-crossview.mjs (+ shared scripts/lib/crossview-shape.mjs),
...
- **bd228911** "Segment the crossview weight bars by sub-element composition"
  User request: bars broken into sub-sections, biggest-left, shade gradient. Added
  `segments` to crossview.json rows; SegmentedBar with descending opacity + thin-tail merge
...
- **c825efd3** "Replace flat chunk groups with a recursive chunk tree"
  ...
  single-child hoisting at group roots, CrossViewChunkTree bars-with-sub-bars (sibling
  scaling, %-of-Atlas, segments mirror children). Removed groups/primes/executors from
  crossview.json (still computed for md docs). Revealed: Spark's Supply Side primitives
...
- **(this commit)** "Add feature folder: continuation prompt + work log"
  docs/features/atlas-crossview/{PROMPT.md,LOG.md} — recovery point for context loss; user
  asked for commit-cadence + logging discipline going forward. Also saved a pointer
  memory (auto-memory: project_atlas_crossview_branch) so future sessions find this folder.

- **(next commit)** "Guard /crossview against artifact/app version skew"
  User hit "page failed to load / reading 'map'": tab open across the chunkTree change —
  module-cached pre-chunkTree crossview.json + HMR-swapped new components. Hard refresh
  fixes the tab; real fix: loadCrossView now validates the artifact shape (chunkTree/
  ...
  predate crossview.json and lack it — harmless, nothing references them; 404 there would
...
- **(next commit)** "Bust immutable cache on crossview.json schema changes"
  ...
  keep the year-cached old artifact. Fix: SCHEMA_V const in src/lib/crossview.ts,
  fetched as crossview.json?v=N — bump in the same commit as any breaking CrossViewData
...
- **(next commit)** "Skip wrapper levels; unify scope view with the chunk tree"
  ...
  CrossViewScopeMass (its children were whole-Atlas-scaled while parents were
  sibling-scaled — mixed visual grammar). "Doc mass by scope" now renders scopeTree
  through the same recursive CrossViewChunkTree (rootDocNo prop shows A.N on roots).
  crossview.json: scopes→scopeTree (SCHEMA_V=3 per the immutable-cache rule). Side
...
- **(next commit)** "Self-heal crossview schema mismatch with one auto-reload"
  ...
  window.location.reload() (sessionStorage latch "crossview-schema-reloaded" prevents
...
Deliverable: docs/crossview/concepts.md — Part I catalog (A meta / B lifecycle /
...
- **(next commit)** "Add Concepts tab to /crossview"
  Concept catalog is now in-app: /crossview/concepts renders docs/crossview/concepts.md
...
- **(next commit)** "Add DR partner onboarding checklist doc"
  docs/crossview/dr-partner-onboarding.md — operational checklist from an ask-atlas
  ...
  for a future /crossview guides section.
...
Wrote docs/features/atlas-crossview/CONCEPTS-AUDIT.md: 4-tier evidence rubric
...
- **(next commit)** "Add Audit tab to /crossview"
  User wants the audit plan readable in-app: /crossview/audit renders
  CONCEPTS-AUDIT.md. Refactored the ?raw markdown rendering into shared
  CrossViewMarkdown.tsx (components map + UUID linkification); CrossViewConcepts.tsx
  now exports thin CrossViewConcepts + CrossViewAudit wrappers. Route + tab wired.
  No patch-notes change (same unreleased CrossView feature).
...
- **(next commit)** "Move CrossView under /reports as the Atlas CrossView report"
  User request; done per .claude/skills/new-report/SKILL.md checklist. Routes
  /crossview* → /reports/crossview* (legacy URLs redirect via wouter Redirect);
  ReportId "crossview"; REPORT_TITLES entry; ReportsIndex card (General Reports);
  report_view tracked on mount; title "Atlas CrossView: ..."; DownloadCsvButton
  on Shape (full chunk-tree export via new pure flattenChunkTree/
  crossviewChunksToCSV in src/lib/crossview.ts + colocated crossview.test.ts, 3
  ...
  routes line now outdated on /crossview paths — superseded by this LOG entry.

## 2026-07-23 — pushed & PR opened

Branch atlas-crossview pushed to origin (github-darkstar alias, darkstar-covenant).
...
- **(next commit)** "Chunk tree: full child listing + label tooltips"
  ...
  7,749 nodes, ~35KB → ~508KB gz (lazy-loaded on the crossview route only;
...
- **(next commit)** "Add nested treemap chunk map to Shape"
  ...
  rendering in CrossViewTreemap.tsx (4 levels deep, minArea cutoff; single
...
- **cee2a2bb + 3b34fb1d + fix**: treemap chunk map shipped; remote had a main
  ...
  their date + our revised bullet), and fixed a merge-mangled legacy /crossview
...
- **Branch cleanup pass** (derived-artifact audit): removed `scripts/aux/atlas-shape.mjs`
  and its outputs `docs/crossview/{toc,shape,glossary}.md`. They were never wired into any
  pnpm script and nothing in the app read them — `/crossview`'s Shape/Contents/Glossary tabs
  already read `crossview.json`/`glossary.json` directly (confirmed by grep before deleting).
  Updated the stale references in `docs/atlas-map.md`, `docs/plans/atlas-crossview.md`,
...
- **Removed the Contents tab** (user decision: superseded by Shape's "Doc mass by scope",
  ...
  below Overlay chunks. Deleted `CrossViewContents.tsx`; dropped `toc` from `crossview.json`
  ...
  from `crossview-shape.mjs` (chunk trees run on the semantic doc_no tree; the field-pick in
  `build-crossview.mjs` is gone — computeCrossView returns exactly the shipped shape).
  `/reports/crossview/contents` now redirects to `/reports/crossview`. Revised the unreleased
  patch-notes bullet + reports-index card copy (no new bullet — same unreleased feature).

- **Dev-doc consolidation → `analyst-crossview` skill.** The session docs in this folder
  dissolved into `.claude/skills/analyst-crossview/SKILL.md` (auto-surfaced to every future
  ...
  Moves: `docs/crossview/dr-partner-onboarding.md` → the skill's
  `references/dr-partner-onboarding.md` (worked exemplar of quote-or-gap);
  CONCEPTS-AUDIT.md → `docs/crossview/concepts-audit.md` (it ships in-app via the Audit
...
- **Folded `crossview.json` away — shape is now computed client-side.** Measured the
  artifact at 2.0 MB raw / 528 KB gz (38% of docs.json gz) while being a pure projection
  of docs.json + a glossary term count — for the common reader→crossview path it *added*
  transfer. Ported `scripts/lib/crossview-shape.mjs` → `src/lib/crossviewShape.ts`
  (byte-identical output verified against the artifact before deletion); `loadCrossView`
  now derives `CrossViewData` from `loadAtlas` + `loadGlossary` per base (crossview works
  over previews now — previews never bundled crossview.json). Deleted `build-crossview.mjs`,
  `crossview-shape.mjs`, the `SCHEMA_V`/skew-reload machinery, and the wiring in
  package.json build chain, Dockerfile, dev-preflight, build-manifest, bundle-store
  allowlist, atlas-updater refresh, .gitignore. Added `crossviewShape.test.ts` (semantic
  ...
  reusing `computeCrossView` against server `Indexes` — fills the doc-mass/orientation
...
- **MCP `atlas_describe` stats section** (the follow-up above, done; named `stats`,
  ...
  `src/server/tools-stats.ts` reuses `computeCrossView` against `ix.docMap` (ChunkNode/
  CrossViewData types moved into `src/lib/crossviewShape.ts` so the server imports no
  DOM code; `crossview.ts` re-exports), WeakMap-memoized per Indexes (updater swap
